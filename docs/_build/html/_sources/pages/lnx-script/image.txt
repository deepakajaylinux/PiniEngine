...
.. function:: [이미지 아이디,파일명,위치="화면중앙",효과="업페이드",효과시간=1,크기="원본크기",북마크이동,스크린샷노출="예",GUI="아니오"]
   ...
   :param 스크린샷노출: :ref:`함수_스크린샷` 함수를 사용하여 스크린샷을 파일로 저장할 경우 해당 이미지가 스크린샷에 포함되게 할 것인지에 대한 여부를 정할수 있습니다.
   :type 스크린샷노출: :ref:`자료형_여부`
   
   :param GUI: :ref:`자료형_여부` 를 전달해주어 해당 이미지가 최상단에 보일것인지를 결정할 수 있습니다. 만약 "예"를 전달한다면 해당 이미지는 어떤 이미지 보다도 가장 앞쪽에 보이게 됩니다. 만약 ``GUI`` 인자를 "예"로 전달한 2개의 이미지가 있다면 나중에 호출된 이미지 함수에 해당하는 이미지가 더 앞쪽에 보이게 됩니다.
   :type GUI: :ref:`자료형_여부`
