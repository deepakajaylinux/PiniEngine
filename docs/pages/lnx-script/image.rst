...
.. function:: [이미지 아이디,파일명,위치="화면중앙",효과="업페이드",효과시간=1,크기="원본크기",북마크이동,스크린샷노출="예",GUI="아니오"]
   ...
   :param 문자열 오버롤: (PC에서만 동작하는 인자 입니다.) 마우스 오버롤시 바뀌게 될 확장자를 포함한 이미지의 파일명을 전달해야 합니다. 마우스 커서가 해당 이미지 위쪽에 닿는 순간 ``오버롤`` 인자로 넘어온 이미지로 바뀌게 됩니다. 그리고 다시 마우스 커서가 해당 이미지를 벗어나는 순간 ``파일명`` 인자로 넘어온 원래의 이미지로 돌아가게 됩니다.
